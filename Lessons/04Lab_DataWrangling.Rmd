...
> Add notes here: It can help us look at various biological processes, such as photosynthesis. We can use these metrics to understand general health of lake systems.
...
NTL.phys.data.Tuesday.90to99 <- NTL.phys.data %>%
  filter(year4 <= 1999 & year4 >= 1990 & lakename == "Tuesday Lake")


```
Question: Why don't we filter using row numbers?

> Answer: Because the row numbers are not static and you don't necessarily know what data is where in the data frame. Also for bigger datasets this is inefficient.
...
NTL.phys.data.Tuesday.90to99.July <- NTL.phys.data %>%
  filter(year4 <= 1999 & year4 >= 1990 & lakename == "Tuesday Lake" & daynum >= 183 & daynum <= 212)
...
NTL.phys.data.Tuesday.90to99.July.surface <- NTL.phys.data.Tuesday.90to99.July %>% 
  filter(depth == 0) %>% 
  group_by(sampledate) %>% 
  dplyr::summarise(Mean_Temp_C = mean(temperature_C))
...
NTL.phys.data.irradiance.gather <- gather(NTL.phys.data, "Irradiance_Type", "Irradiance_Value", irradianceWater:irradianceDeck) 

NTL.phys.data.irradience.pivot <- pivot_longer(NTL.phys.data, irradianceWater:irradianceDeck, names_to = "Irradiance_Type", values_to = "Irradiance_Value")
