...
huc8_sf_UppNeuse <- st_read(here('Data/Spatial/NCHUC8.shp')) %>% 
  filter(SUBBASIN == "Upper Neuse")

#Create a map

mapview(huc8_sf, col.regions="grey") + mapview(huc8_sf_UppNeuse, col.regions="yellow",col="blue")  
...
Wind_Energy_Join <- huc8_sf %>% 
  left_join(Wind_Energy, by = "HUC_8") 

#View the outputs
mapview(Wind_Energy_Join)
...
unique(Wind_Energy_Join$DWQ_Basin)

#Summarize on DWQ Basin value
huc2_sf <- Wind_Energy_Join %>% 
  group_by(DWQ_Basin) %>% 
  summarize(Area_ac = sum(ACRES))
  

#Map the data
mapview(huc2_sf, zcol = "Acres_ac")
...
triCo <- counties_sf_utm %>% 
  ...
  st_transform(st_crs(huc8_sf))

  
#Grab the intersecting HUC_8s

trangle_intersect <- huc8_sf %>%  
  st_filter(triCo, .predicate = st_intersects)



#Intersect the HUC_8s
triangle_intersect_sf <- huc8_sf %>% st_intersection(triCo)
...
counties_30km <- st_buffer(theSite_sfc_transformed, 30000)
counties_select <- counties_sf_utm[counties_30km,]
