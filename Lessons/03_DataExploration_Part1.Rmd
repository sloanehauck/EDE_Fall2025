...
> Answer: When loading the packages, it will list any core packagaes and conflicts that occur. For example, if multiple packagaes have "filter()". 
...
install.packages(c("tidyverse", "dplyr", "ggplot2", "here"))
library(tidyverse)
library(dplyr)
library(ggplot2)
library(here)
...
> ANSWER: The README lives in the metadata subfolder of the data folder. By keeping supporting data in a metadata folder, it is less likely to get lost, so it is more accessible for reproduction purposes, and it keep sthe folders organized by file type and use. The README contains valuable information about the dataset that can help others assess the dataset in a similar way, or understand why certain fields were used for analysis.
...
> Answer: They appear as NA if it is a numeric column, otherwise they appear blank.
...
> Answer: If you want to know mean discharge for days where data was collected, you should omit the NAs, but if you want to know the mean for an entire year, the NAs shoudl not be removed because they represent a data point where no data was collected.
...
```{r Fixing dates}
# This changes the format to be year-month-day
USGS.flow.data$datetime <- format(USGS.flow.data$datetime, "%y%m%d")

# Where dates were erroneously put as 2000's it replaces the 20 with 19 where 19 starts the value
create.early.dates <- (function(d) {
       paste0(ifelse(d > 191226,"19","20"),d)
       })
#applies the above function to the dates in the table
USGS.flow.data$datetime <- create.early.dates(USGS.flow.data$datetime)

#reformats the data to be YYYY-MM-DD
