...
ggplot(EPAair, aes(x=factor(Month, levels = 1:12, labels = month.abb))) +
  geom_bar() +
  xlab("month") + 
  scale_x_discrete(drop = FALSE)
...
PeterPaul.chem.nutrients_surface <- PeterPaul.chem.nutrients %>% 
  filter(depth == 0) %>% 
...
PeterPaul.chem.nutrients_surface
#2. 
...
PeterPaul.chem.nutrients_depth <- PeterPaul.chem.nutrients %>% 
  ggplot(aes(x=sampledate, y=temperature_C, color=depth)) +
  geom_point(size = 0.5) +
  scale_color_viridis(option = "magma", direction = -1) +
  scale_x_date(date_breaks = "5 years",
               date_labels = "%Y") +
  labs(title = "Temperature (C) by Date across Depths",
       y = "Temperature (C)",
       x = "Year",
       color = "Depth")
  
PeterPaul.chem.nutrients_depth
...
ggplot(EPAair, aes(x=PM2.5, y=Ozone, colour = mean_Lat)) +
  geom_point(alpha=0.5) +
  geom_smooth(method = lm) +
  labs(title = "Relationship Between Ozone and PM2.5",
       color = "Mean Latitude")
...
ggplot(EPAair, aes(x=Year, y=PM2.5, fill=factor(Year))) +
  geom_violin()
