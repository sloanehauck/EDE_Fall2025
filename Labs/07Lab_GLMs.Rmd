...
```{r}
#Exercise 1:
ozone1 <- ggplot(EPAair, aes(x = Ozone, fill = factor(Year))) +
...
ozone2 <- ggplot(EPAair, aes(x = factor(Year), y = Ozone, fill = factor(Year))) +
  geom_boxplot() +
  labs(x="Year", y="Concentration", fill = "") +
...
ozone3 <- ggplot(EPAair, aes(x=factor(Year), y=Ozone)) +
...
EPAair_subset <- EPAair %>% 
  select(mean_Lat:Ozone) %>% 
  na.omit()

EPAairCorr <- cor(EPAair_subset)
...
Ozone_regression <- lm(data = EPAair_subset, PM2.5 ~ Ozone + Year + Month + mean_Lng + mean_Lat)

step(Ozone_regression)
...
for (i in length(Litter.Totals)) {
  treatgroups = assign_group(Litter.Totals$plotID)
}
