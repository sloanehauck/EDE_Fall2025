...
electricity_2024 <- data.frame("State" = the_states,
                               "Price" = the_price,
...
#4 Link to the web site using read_html
the_url <- "https://www.eia.gov/electricity/state/archive/2021/"
the_website <- read_html(the_url)

#5&6 Locate elements and read their text attributes into variables
the_states <- the_website %>% html_nodes("td:nth-child(1)") %>% html_text()
the_price <- the_website %>% html_nodes("td:nth-child(2)") %>% html_text()
the_capacity <- the_website %>% html_nodes("td:nth-child(3)") %>% html_text()
net_generation <- the_website %>% html_nodes("td:nth-child(4)") %>% html_text()
...
#7 Construct a dataframe from the values

electricity_2021 <- data.frame("State" = the_states,
                               "Price" = the_price,
                               "Capacity" = the_capacity,
                               "Net Generation" = net_generation,
                               "Total Retail" = total_retail) %>% 
...
electricity_2017 <- scrape.it(2017)

# Map the function to scrape data from 2021 to 2023
years <- rep(2021:2023)
the_dfs <- map(years,scrape.it)
electricity_2021_to_2023 <- bind_rows(the_dfs)
