...
Rainfall_Plot <- ggplot(BR_complete, aes(x = Date, y = Rainfall)) +
  geom_line() +
...
Temperature_Plot <- ggplot(BR_complete, aes(x = Date, y = Temperature_C)) +
  geom_line() +
...
Rainfall_ts <- ts(BR_complete$Rainfall, start = c(f_year, f_month), 
                  frequency = 12)

Temperature_ts <- ts(BR_complete$Temperature_C, start = c(f_year, f_month), 
                     frequency = 12)
...
Temperature_Components <- mutate(Temperature_Components, 
                              Temperature = BR_complete$Temperature_C,
                              Date = BR_complete$Date)
...
Rainfall_Trend <- trend::smk.test(Rainfall_ts)
...
Rainfall_Trend_NoSeason <- Rainfall_ts - Rainfall_Decomposed$time.series[,1]

Temperature_Trend_NoSeason <- Temperature_ts - Temperature_Decomposed$time.series[,1]

Rainfall_Components <- mutate(Rainfall_Components, 
                              Nonseasonal = Rainfall_Trend_NoSeason)

Temperature_Components <- mutate(Temperature_Components,
                                 Nonseasonal = Temperature_Trend_NoSeason)

ggplot(Rainfall_Components) +
  geom_line(aes(y = Rainfall, x = Date), size = 0.25) +
  geom_line(aes(y = Nonseasonal, x = Date), color = "lightblue") +
  ylab("Temperature")
...
Rainfall_NoSeason_Trend <- trend::mk.test(Rainfall_Trend_NoSeason)
Rainfall_NoSeason_Trend

Temperature_NoSeason_Trend <- trend::mk.test(Temperature_Trend_NoSeason)
Temperature_NoSeason_Trend

```

What would we conclude based on these findings? 

> Answer: The data has a seasonal trend.
